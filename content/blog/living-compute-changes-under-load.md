...
In the rebuilt twin each organoid has a life of its own. That life drains with time and, in the twin, with every stimulation pulse, at a pace that differs between organoids, and when it falls low enough the organoid goes silent and stays silent, and rest does not return it. The recordings show that elapsed time alone is enough, because tissue declined, and organoids fell silent, through long stretches with no stimulation at all. Whether stimulation adds to that decline they could not settle, since the tissue that was stimulated far more also differed in age, temperature and care. The twin therefore ages with time and also charges life for every pulse, and I treat that per pulse charge as a modeling choice rather than a finding. The pace is compressed so that a trend which takes real tissue days is visible within a lab session. Electrodes are very unequal, with well under half of them lively, some firing rarely and about a third recording nothing but artifact, and spontaneous firing comes in bursts that a whole organoid shares, rather than evenly. A stimulated electrode drives only other electrodes of its own organoid, so nothing propagates between organoids, and that fixed wiring acts as the reservoir because different inputs excite different channel combinations. Evoked spike counts are Poisson distributed with a mean that scales with the organoid's remaining life. In the twin the evoked response fades faster than spontaneous firing does, which one tissue showed and another did not. At the stimulation settings the lab uses, every pulse also leaves an artifact on all 32 channels for about ten milliseconds, and many of those artifacts are small, so a reader that filters by amplitude alone fails and the first ten milliseconds have to be discarded. With gentler stimulation real tissue showed far less artifact, and the twin scales it accordingly.

Two things in the twin are kinder than the tissue it imitates, and both exist so that a lab can be built on it. Its evoked responses are far clearer than real ones, clear enough for a simple readout to work, and it keeps a mild tiredness that rest does relieve, with a 20 second time constant. The recordings give that only modest support, a small dip in the response when stimulations follow each other within a few tenths of a second, and none at intervals of several seconds. A pulse above 20 microamperes removes a large share of an organoid's remaining life at once, which models damage and for which I have no data at all. These magnitudes are illustrative, and only the direction of each effect is meant to be realistic.

### Task, Readout and Agents

The task was to recognize four shapes drawn on a 4 by 4 grid, which maps directly onto the trigger pattern, so each lit cell fires one electrode with a fixed biphasic pulse of 5 microamperes. The readout counts spikes per channel from 10 to 200 milliseconds after the stimulus, discards artifacts and subtracts spontaneous baseline activity. A nearest centroid rule on cosine similarity, trained per array and checked against a negative control, then classifies the resulting vector. Chance accuracy is 25%, the trained readout names roughly two shapes in three, and confidence is the margin between the best and second best similarity scores. That is already far better than I would expect from tissue, where the same kind of readout separated stimulation patterns only modestly, and where part of the apparent separation turned out to be drift in the tissue rather than the stimulus.
...
The most important limitation is that this study used a digital twin and no living tissue, so every figure is an output of a software model. The twin follows trends I saw in recordings of real organoids, but those recordings are few, they come from experiments designed for other purposes and without unstimulated controls, and while they establish that tissue declines with time alone, they could not tell me whether stimulation adds to it. The twin's per pulse charge is therefore an assumption, and so is its damage threshold, for which I have no data, while its mild recoverable tiredness has only modest support. Its evoked responses are much clearer than those of real tissue, so the readout's accuracy says nothing about what tissue can do. The pace of decline is compressed, the magnitudes are illustrative, and the 2.5 second interval was chosen because it suited the model. The numbers are therefore not benchmarks of organoid computing or of the gateway.
...
I should also record that this article was revised, since its first draft described a twin whose tissue tired in seconds and recovered with rest, and reported that pacing restored accuracy. The recordings supported at most a small effect of that kind, I rebuilt the model, and the result changed from one about correctness to one about how fast a finite resource is spent. Part 2 of this series will report the analysis of those recordings in its own right, with the agreement of the people who shared them: what declines and how, what a simple readout can and cannot recover from real tissue, and which questions the recordings leave open.
